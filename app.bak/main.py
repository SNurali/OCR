...
from app.tracing import trace_context, inject_trace_headers, extract_trace_id
...
async def log_requests(request: Request, call_next):
    ...
    if trace_id:
        trace_context.continue_trace(trace_id, f"{request.method} {request.url.path}")
    else:
        trace_id = trace_context.start_trace(f"{request.method} {request.url.path}")
...
for route in analytics_router.routes:
    if hasattr(route, "dependencies"):
        route.dependencies.append(Depends(limiter.limit("60/minute")))
