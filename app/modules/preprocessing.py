...
def preprocess_uzbek_passport(image: np.ndarray) -> np.ndarray:
    """
    Специализированная предобработка для узбекских паспортов.
    
    Особенности:
    - Удаление фона с водяными знаками
    - Усиление контраста для бледного текста
    - Коррекция цвета для лучшей читаемости
    ...
    # Конвертация в LAB для улучшения контраста
    ...
    # Sharpening для улучшения четкости текста
    ...
    denoised = cv2.fastNlMeansDenoisingColored(sharpened, None, 5, 5, 7, 15)
    
    # Коррекция цвета для удаления желтого/зеленого оттенка
    hsv = cv2.cvtColor(denoised, cv2.COLOR_BGR2HSV)
    h_channel, s_channel, v_channel = cv2.split(hsv)
    
    # Уменьшение насыщенности для более нейтрального фона
    s_reduced = cv2.multiply(s_channel, 0.7)
    hsv_adjusted = cv2.merge([h_channel, s_reduced, v_channel])
    color_corrected = cv2.cvtColor(hsv_adjusted, cv2.COLOR_HSV2BGR)
    
    logger.info(f"Uzbek passport preprocessing applied: {scaled.shape} → {color_corrected.shape}")
